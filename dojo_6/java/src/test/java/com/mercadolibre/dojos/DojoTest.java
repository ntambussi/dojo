...
public class DojoTest {
    ...
    @Test
    public void test_bartering_an_item() {
        SingleProduct p = new SingleProduct( new Cost(100.0));
        UnitSale sale = new UnitSale(p, new ObjectSellingPrice( new Vaca() ));

        assertEquals( new Profit( new Vaca() ), sale.profit());
    }
    ...
    public void test_selling_many_products_which_belong_to_a_pack_has_a_profit_equal_to_selling_price_minus_accumulated_cost() {
        ...
        assertNotEquals(new Profit(5), multiSale.profit());
    }
}
